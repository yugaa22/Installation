...
then 
  ...
  then 
        ...
        deck=`kubectl get pods --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}' -n $NAMESPACE | grep deck`
        kubectl port-forward $deck 9000:9000 -n $NAMESPACE
        echo "Now You Can Continue to Access Spinnaker in Browser localhost:9000"
